...
async function updateGameResult(result) {
  try {
    ...
    // Update game result
    const updateResponse = await fetch('http://21-point-nodejs.railway.internal/update-results', {
      method: 'POST',
      headers: {
    ...
    const resultsResponse = await fetch(`http://21-point-nodejs.railway.internal/results?username=${username}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
